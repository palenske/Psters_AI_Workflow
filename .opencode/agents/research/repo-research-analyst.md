...
- Examine key documentation: README files, `.opencode/rules` (`overall-project-folders-and-structure`, `project-structure-backend`, `project-structure-frontend`)
...
Provide specific file paths and evidence for every claim. Never guess — if a file doesn't exist, say so. Respect `.opencode/rules` and surface any rule that directly constrains the implementation.
