...
Este documento descreve os hooks que existiam no plugin original para Cursor, mas que **não foram migrados** para o Windsurf devido a incompatibilidades no sistema de extensões.
...
**Windsurf** usa um sistema de extensões diferente baseado em `extensions.json` com hook points específicos (before_plan, after_plan, before_work_plan, after_work_plan, before_work, after_work).

Os scripts `.mjs` dos hooks do Cursor dependem de APIs específicas do Cursor (`CURSOR_PLUGIN_ROOT`, eventos do editor) que não existem no Windsurf.
...
## 🔄 Possíveis Alternativas no Windsurf

### Sistema de Extensões do Windsurf

O Windsurf usa `extensions.json` com hook points diferentes:
...
| Hook do Cursor | Alternativa no Windsurf | Status |
|----------------|-------------------------|--------|
...
No Windsurf, essas disciplinas devem ser mantidas **manualmente** ou via:
