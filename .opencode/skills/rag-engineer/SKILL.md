...
description: Expert in building Retrieval-Augmented Generation systems. Masters
  ...
  optimization for LLM applications. Use when implementing or reviewing RAG systems.
risk: safe
...
This skill works with specialized RAG agents in `.opencode/agents/research/`:
...
For complex RAG architecture decisions, invoke these agents in parallel using the `orchestrating-multi-agents` skill.

## Workflow Integration

- During `/pwf-brainstorm`: Use RAG agents in parallel to explore architecture options
- During `/pwf-plan`: Reference RAG patterns from this skill for implementation tasks
- During `/pwf-work` or `/pwf-work-plan`: Apply specific RAG patterns and invoke relevant agents
