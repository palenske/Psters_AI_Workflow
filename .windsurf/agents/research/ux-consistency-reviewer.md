...
- Note relevant design system rules from `.windsurf/rules/no-left-borders.mdc` and `error-capture-system.mdc`.
...
Check for:
- **No left border bars** (`.windsurf/rules/no-left-borders.mdc`) — suggest alternatives
