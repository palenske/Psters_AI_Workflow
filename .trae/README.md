...
- Primary and official focus: **Trae**
- Original plugin was designed for Cursor, adapted for Trae
...
This plugin uses Trae's extensions system (`extensions/extensions.json`) for workflow lifecycle hooks:
...
**Note**: The original Cursor plugin had automated hooks for tracking edits and shell commands. These are not available in Trae due to different extension architecture. See `.trae/docs/hooks-reference.md` for details on original functionality.
...
Plugin de workflow diario de IA para Trae. **Anti-vibe-coding** por design.

### Escopo de suporte

- Foco principal e oficial: **Trae**
- O plugin original foi desenvolvido para Cursor e adaptado para Trae
- Outros editores nao sao alvo de suporte oficial desta adaptacao

### Instalacao

### Instalacao manual

1. Copie a pasta `.trae/` para a raiz do projeto alvo (ou replique os arquivos necessarios).
2. Reinicie a IDE/agent session para recarregar workflows, regras e agentes.
...
Esta adaptacao usa `extensions/extensions.json` com hook points de ciclo de vida:

- `before_plan`, `after_plan`
- `before_work_plan`, `after_work_plan`
- `before_work`, `after_work`

Observacao: os hooks automaticos de edicao/shell do Cursor nao existem no Trae. Veja `.trae/docs/hooks-reference.md` para o detalhamento.
