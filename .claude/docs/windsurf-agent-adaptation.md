# Adaptação de Agentes para Windsurf

Este documento explica as diferenças entre o sistema de agentes do Cursor e do Windsurf, e como adaptar os workflows corretamente.

---

## Problema

O plugin original foi desenvolvido para **Cursor**, que possui uma ferramenta chamada **Task** para invocar subagentes com `subagent_type: generalPurpose`. O **Windsurf não possui essa ferramenta**.
...
## Solução: Adaptação para Windsurf
...
**Windsurf (adaptado):**
```markdown
Execute research agents directly (Windsurf does not use Task tool):
...
### 2. **Abordagem de Invocação de Agentes no Windsurf**

O Windsurf permite invocar agentes de duas formas:
...
### 3. **Paralelização no Windsurf**

O Windsurf suporta chamadas paralelas de ferramentas. Para executar múltiplos agentes em paralelo:
...
Para adaptar um workflow do Cursor para o Windsurf:
...
- [ ] Manter instruções de paralelização (Windsurf suporta)
...
Os seguintes hooks do Cursor **não funcionam** no Windsurf:
...
**Versão**: 1.0  
**Status**: Documento de referência para adaptação Cursor → Windsurf
